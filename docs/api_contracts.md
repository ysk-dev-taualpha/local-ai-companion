...
| メソッド | シグネチャ | 説明 |
|----------|-----------|------|
| `NewWebSocketHub(pythonClient, stateMachine, requestTimeoutMs, allowedOrigins)` | `*WebSocketHub` | Hub を生成 |
...
- `CheckOrigin` は `websocket.allowed_origins` で制限される
- `websocket.allowed_origins` が空の場合は、Origin 未指定の非ブラウザ接続と localhost（`localhost` / `127.0.0.1` / `::1`）のみ許可する
- `websocket.allowed_origins` に値がある場合は、完全一致する Origin のみ許可する（例: `http://localhost:3000`）
- 同時接続は goroutine-safe に管理される
- 同一 WebSocket 接続への書き込みは接続ごとの mutex で直列化される
- 会話状態の遷移は共有 StateMachine に対して直列化される
