...
- 会話状態の遷移は共有 StateMachine に対して直列化される（内部で sync.Mutex により排他制御）
...
**スレッドセーフ**: 全メソッド（`Current`, `Transition`, `Reset`）は内部で `sync.Mutex` による排他制御を行い、複数 goroutine からの同時呼び出しに対して安全です。
